...
class App(ROOT):
    def __init__(self):
        ...
        height = min(455, max(355, int(sh * 0.56)))
        self.geometry(f"{width}x{height}")
        self.minsize(590, 345)
        ...
        self._pending_start_after_choice = False
    ...
    def _ui(self):
        ...
        self.run_btn = self._btn(right, "İşlemi Başlat", self.start, primary=True, width=12)
        ...
        self.prog = ttk.Progressbar(bottom, style="Work.Horizontal.TProgressbar", mode="indeterminate")
        self.prog.pack(fill="x", pady=(0, 3))
        tk.Label(bottom, textvariable=self.status, bg=CARD, fg=MUTED, font=(self.ff(), 8)).pack(side="left")
    ...
    def _prompt_action(self, start_after_choice=False):
        self._pending_start_after_choice = bool(start_after_choice)
        ...
        tk.Label(fr, text="Word: satır düzeltme • PDF/görsel: Word'e aktarım", bg=CARD, fg=MUTED, font=(self.ff(), 8)).pack(anchor="w", padx=10, pady=(0, 8))
        row = tk.Frame(fr, bg=CARD)
        row.pack(fill="x", padx=10, pady=(0, 9))

        def choose(value: str):
            should_start = self._pending_start_after_choice
    ...
    def add_paths(self, paths, ask_action=False):
        ...
        self._refresh()
        if ask_action:
            self._prompt_action(start_after_choice=False)
    ...
    def _refresh(self):
        ...
        self.summary.set(f"{len(self.items)} öge seçildi" if self.items else "Henüz dosya veya klasör seçilmedi")
    ...
    def start(self):
        ...
        if action not in {"repair", "convert"}:
            self._prompt_action(start_after_choice=True)
            return
        ...
        self._set_busy(True)
        self.status.set("İşlem başladı… İptal düğmesi her zaman kullanılabilir.")
    ...
    def _work(self, action, targets, include_images):
        ...
        for index, p in enumerate(targets, 1):
            if self.cancel_event.is_set():
                cancelled = True
                break
            self.q.put(("status", f"{index}/{len(targets)} • İşleniyor: {p.name}"))
            try:
                if action == "repair":
                    r = repair_file(p)
                    if r.skipped:
